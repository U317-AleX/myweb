...
## 共识机制
区块链的**共识机制（Consensus Mechanism）**是指网络中所有节点在没有中心化控制的情况下，如何就某一数据状态（如新区块）达成一致的协议。它是去中心化系统的核心，它能确保所有节点最终拥有一致的区块链副本，并且发布的交易都是合法的。
...
这和 root hash 的验证有关系。假设这个 miner 不验证就直接把这个区块添加到区块链上，如果这个区块中的交易的不合法的，那么这个 miner 所拥有节点上记录的三颗 Merkel Tree 的 root hash 就和其他老实验证 miner 所拥有的节点上记录的 root hash 分道扬镳了。如果不出意外，**那么这个不老实的 miner 所发布的区块就永远通不过验证了！** 

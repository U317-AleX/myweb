...
## 区块链的共识机制
**共识机制（Consensus Mechanism）**是指分布式系统如何就某一数据状态达成一致的协议。大多数需要对外展现出一致性的分布式系统都需要**共识机制（Consensus Mechanism）**或**共识算法（Consensus Algorithm）**来达成这一目标。比如 raft 集群需要使得每个节点的 state machine 和 log 保持一致，这时候就需要共识机制。
